...
class Command(BaseCommand):
    help = 'Generate static sitemap XML files'

    def handle(self, *args, **options):
        output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'static_sitemaps')
        os.makedirs(output_dir, exist_ok=True)

        base_url = 'https://zenthir.com'

        # === PROVIDERS (with pricing only) ===
        ...
        self.stdout.write(f'  {len(provider_files)} provider sitemap files')

        # === PROCEDURES ===
        self.stdout.write('Generating procedure sitemap...')
        procedures = list(Procedure.objects.filter(
            pricing_records__isnull=False
        ).distinct().values_list('slug', flat=True))
        filepath = os.path.join(output_dir, 'sitemap-procedures.xml')
        with open(filepath, 'w') as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
            f.write('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n')
        ...
        # === MARKET PAGES ===
        self.stdout.write('Generating market sitemap...')
        consumer_slugs = [
        ...
        # === STATIC ===
        filepath = os.path.join(output_dir, 'sitemap-static.xml')
        with open(filepath, 'w') as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
            f.write('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n')
        ...
        with open(filepath, 'w') as f:
            ...
            for pf in provider_files:
                f.write(f'<sitemap><loc>{base_url}/sitemaps/{pf}</loc></sitemap>\n')
            f.write(f'<sitemap><loc>{base_url}/sitemaps/sitemap-procedures.xml</loc></sitemap>\n')
            f.write(f'<sitemap><loc>{base_url}/sitemaps/sitemap-locations.xml</loc></sitemap>\n')
            for mf in market_files:
                f.write(f'<sitemap><loc>{base_url}/sitemaps/{mf}</loc></sitemap>\n')
            f.write(f'<sitemap><loc>{base_url}/sitemaps/sitemap-static.xml</loc></sitemap>\n')
